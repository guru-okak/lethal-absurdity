...
const bgLethalV3Url = new URL('../ui/bg/bg-lethal_v3.png', import.meta.url).href
...
function RoundScreen({ room, role, situationDraft, actionDraft, onSituation, onAction, onSubmitSituation, onSubmitAction, onWithdrawAction, onTimeout, onActionRevealContinue, onRetryAiGeneration, onNextRound, isSpeaking, onBack }) {
  ...
  if (room.phase === 'reveal') return <ScreenFrame eyebrow="Вердикт" title="Решение ведущего" onBack={onBack}><section className="verdict-card"><style>{`
          .verdict-reveal-shell { background: url('${bgLethalV3Url}') center / cover no-repeat fixed; box-sizing: border-box; display: flex; flex-direction: column; min-height: 100vh; overflow-x: hidden; padding: 24px 48px; }
...
function ActionsPhaseScreen({ room, role, actionDraft, onAction, onSubmitAction, onWithdrawAction, canSubmitTimeout, remaining, hasSubmittedAction, onBack }) {
  ...
  return <main className="app-shell actions-screen-shell">
    <style>{`
      .actions-screen-shell { background: url('${bgLethalV3Url}') center / cover no-repeat fixed; display: flex; flex-direction: column; min-height: 100vh; overflow-x: hidden; padding: 24px 48px; }
...
function SituationCreationPage({ room, isScenarioChooser, situationDraft, onSituation, onSubmitSituation, onBack }) {
  ...
  return <main className="situation-creation-page">
    <style>{`
      .situation-creation-page { background: url('${bgLethalV3Url}') center / cover no-repeat fixed; color: #d4d4d8; display: flex; flex-direction: column; justify-content: space-between; min-height: 100vh; overflow-x: hidden; overflow-y: auto; padding: 24px 48px; position: relative; width: 100%; }
...
function MatchHistoryScreen({ room, role, onAwards, onBack }) {
  ...
  return <ScreenFrame eyebrow="Хроника" title="Хроника Матча" onBack={onBack}><section className="match-history-screen"><style>{`
    .match-history-shell { background: url('${bgLethalV3Url}') center / cover no-repeat fixed; box-sizing: border-box; display: flex; flex-direction: column; min-height: 100vh; overflow-x: hidden; padding: 24px 48px; }
